...
Batch.prototype._setupDataStreams = function() {
  ...
  if (this.job.options.zip) {
    // if zip file, write directly to the stream without serializing
    this._uploadStream = new stream.PassThrough();
    this._uploadDataStream = this._uploadStream;
  } else {
...
Bulk.prototype.load = function(type, operation, options, input, callback) {
  ...
  if (operation.toLowerCase() !== 'upsert' && (typeof input === 'function' || typeof input === 'undefined')) { // options is only for upsert operation
    callback = input;
    input = options;
    options = null;
  }
